 ...
 Future<String> doSomeThing({String eventData}) async {
  ...
  Future<String> resize({int index}) async {
   ...
   IMG.Image image =
      ...
      File resizedPic = await saveResizedPicToJpg(picFile: resized,name: getSavableDateString(DateTime.now()),extension: '.jpg').then((value){
        returnValue = 'Picture has been resized';
        return value;
      });
  ...
  String getSavableDateString(DateTime t) {
    ...
    String returnString =
        '${date.split('-')[2]}_${date.split('-')[1]}_${date.split('-')[0]}';
    return returnString;
  }
}
